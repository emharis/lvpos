    ...
    @foreach($employee as $value)
        <tr>
            ...
            <td>
                ...
                <a class="btn btn-sm btn-info" href="{{ URL::to('employees/' . $value->id . '/edit') }}">{{trans('employee.edit')}}</a>
                {!! Form::open(array('url' => 'employees/' . $value->id, 'class' => 'pull-right')) !!}
                    {!! Form::hidden('_method', 'DELETE') !!}
                    {!! Form::submit(trans('employee.delete'), array('class' => 'btn btn-sm btn-warning')) !!}
                {!! Form::close() !!}
            </td>
        </tr>
    @endforeach
